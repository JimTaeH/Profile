...
<p> Delete the error audio files, fix mispell or wrong word (if see), fix number. First, the error audio files meaning audio with no transcription or audio with no sound nor speech. I am drop all that files from the dataset. Then I am print all transcription and checking the spelling and replace the mispell or wrong word with correct word (this is something manually but grateful this dataset has cleaning before hand). </p>
